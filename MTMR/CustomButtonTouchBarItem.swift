...
class CustomTouchBarItem: NSCustomTouchBarItem, Decodable, NSGestureRecognizerDelegate {
    var align: Align
...
class CustomButtonTouchBarItem: CustomTouchBarItem {
...
//        button.attributedTitle = attributedTitle
    ...
    required init(from decoder: Decoder) throws {
        ...
        self.addAction(try? SingleTapEventAction(from: decoder))
        self.addAction(try? LongTapEventAction(from: decoder))
    }
    ...
    func addAction(_ action: ItemAction?) {
        if action != nil {
            ...
            if action!.trigger == Action.Trigger.longTap {
    ...
    func callActions(for trigger: Action.Trigger) {
        let itemActions = self.actions.filter {
            $0.trigger == trigger
        }
